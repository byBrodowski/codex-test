...
## Готовий `.exe` без збірки
У репозиторії є автоматична збірка через GitHub Actions. Після запуску workflow або при релізі
ви отримаєте готовий `PremiumDock.exe`, який можна одразу запускати без встановлень.

1. Відкрийте вкладку **Actions** → **Build PremiumDock**.
2. Запустіть workflow або дочекайтеся авто-збірки при релізі.
3. Завантажте артефакт `PremiumDock-win-x64` і запустіть `PremiumDock.exe`.

## Як зібрати `.exe` вручну (для розробників)
